...
const OrderCreateForm = () => {
  ...
  const onSubmit = async (data: OrderCreateFormProps) => {
    try {
      ...
      const order = await sdk.client.fetch<any>("/admin/create-cart-order", {
        method: "POST",
        body: orderData,
      });
  ...
  const onSubmitCart = async (data: OrderCreateFormProps) => {
    try {
      ...
      const order = await sdk.client.fetch<any>("/admin/create-order", {
        method: "POST",
        body: orderData,
      });
      if (order) {
                              ...
                              <Combobox
                                ...
                                onChange={(value) => {
                                  ...
                                  if (selectedAddress) {
                                    ...
                                    if (
                                      form.getValues("billing_same_as_shipping")
                                    ) {
                                      form.setValue(
                                        "billing_first_name",
                                        selectedAddress.first_name
                          ...
                          <Form.Control>
                            <CountrySelect
                              ...
                              countries={
                                regions.options?.find(
                                  (region) =>
                                    region.value === form.getValues("region_id")
                                )?.countries
                              }
                          ...
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => {
                              ...
                              if (checked) {
                                ...
                                form.setValue(
                                  "billing_address_2",
                                  shippingData.shipping_address_2
                                );
                                form.setValue(
                                  "billing_company",
